...
def registerAuth():
	...
	if(data):
		...
		cursor.execute(ins, (username, md5password, f_name, l_name))
		conn.commit()
		cursor.close()
		return render_template('index.html')
...
def friends():
	...
	return render_template('friends.html', urFriends = yourFriends, ownFG=ownedFG)

@app.route('/myposts')
def myposts():
	username = session['username']
	cursor = conn.cursor();
	query = '''SELECT ID, content_name, file_path, timest FROM content
	WHERE username = %s
	ORDER BY content.ID DESC'''
	cursor.execute(query, (username))
...
def friendgroups():
	username = session['username']
	cursor = conn.cursor();
	query = '''SELECT group_name, description FROM person Natural Join friendgroup
	WHERE person.username = %s'''
	cursor.execute(query,username)
	ownedFG = cursor.fetchall()
...
if __name__ == "__main__":
	app.run('127.0.0.1', 8886, debug = True)
